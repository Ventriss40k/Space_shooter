...
class Enemy_cruiser(pygame.sprite.Sprite):
    def __init__ (self, x,y, speed):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load("sprites\enemies\enemy_cruiser.png").convert_alpha()
        self.rect = self.image.get_rect()
        self.rect.center = [x, y]
        self.pos = [x, y]
        self.mask = pygame.mask.from_surface(self.image)
        self.speed = speed
        self.turret_pos = [self.rect.centerx- 100, self.rect.centery- 100]
        self.turret = Turret(self.turret_pos[0], self.turret_pos[1])
        turret_group.add(self.turret)
    def update(self):
        self.pos[0] -=self.speed[0]
        self.rect.center = self.pos
        self.turret_pos = [self.rect.centerx - 100, self.rect.centery - 100]

        self.turret.pos = self.turret_pos
...
class Turret(pygame.sprite.Sprite):
    def __init__ (self, x,y,):
        ...
        self.pos = [x, y]
        self.mask = pygame.mask.from_surface(self.image)       
        self.gun = TurretGun(self.rect.centerx, self.rect.centery, self)
        gun_group.add(self.gun)
    def update(self):
        self.gun.update()
        self.rect.center = self.pos

        self.gun.nominal_image_rect.center = self.rect.center

class TurretGun(pygame.sprite.Sprite):
    def __init__ (self, x,y, turret):
        pygame.sprite.Sprite.__init__(self)
        ...
        self.pos = [x, y]
        self.turret = turret
        self.nominal_center = [self.turret.pos[0], self.turret.pos[1]]
        self.mask = pygame.mask.from_surface(self.image)   
        self.angle = 0    
    ...
    def shoot_player(self):
        if self.gun_cooldown == 0:
            ...
            x_start = self.nominal_center[0]
            Y_start = self.nominal_center[1]
            ...
            if player.rect.centery > Y_start:
                ...
                y_sign = 0
            projectile_group.add(RoundProjectile(x_start, Y_start, x_percent, y_percent, x_sign, y_sign))
            # gun cooldown
...
class RoundProjectile(pygame.sprite.Sprite):
    def __init__(self, x, y, x_percent, y_percent, x_sign, y_sign):
        ...
        self.y_percent = y_percent
        self.printed = False
        pygame.draw.circle(self.image, (0, 255, 0), self.circle_center, self.circle_radius)
    def update(self):
        self.pos["x"] += self.x_vel
        self.pos["y"] += self.y_vel 
        self.rect.center =[self.pos["x"], self.pos["y"] ]
...
enemy_group.add(Enemy_cruiser(screen_width, screen_height*0.8, [0.2, 0]))
